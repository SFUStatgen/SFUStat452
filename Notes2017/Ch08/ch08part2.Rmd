...
<!-- for next time, set a seed in this chunk for repro in Ch08part3 -->
...
```{r}
t1.best <- prune.tree(t1,best=4)
summary(t1.best)
hpred <- predict(t1.best,newdata=Heart[-train,],type="class")
table(hpred,Heart[-train,]$AHD)
28/nrow(Heart[-train,])
```
...
table(bag.hpred,Heart[-train,]$AHD)
18/nrow(Heart[-train,])
```
...
bag.hpred <- predict(bag.heart,newdata=Heart[-train,],type="class")
table(bag.hpred,Heart[-train,]$AHD)
17/nrow(Heart[-train,]) # Lowest so far.
```
...
* Text, Fig. 8.8. Not clear what "Test" error is based on, possibly
from a hold-out validation sample.
OOB error is lower than test error. Perhaps because the trees
are based on more data (size $n$ bootstrap samples).