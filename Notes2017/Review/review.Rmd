...
of a new $Y$.
* Based on and estimate $\hat{f}$ of $f$
...
we have not been explicit about
loss functions, but have mentioned zero-one loss
(misclassification error),
logistic loss (logistic regression) and others.
...
* For fixed $x_0$ and $y_0$ **not used to train $\hat{f}$**, 
the expected test error is 
$E(L(Y,\hat{f}(X)|y_0,x_0)$.
* For example, with square error loss we have the test MSE
$E(y_0 - \hat{f}(x_0))^2$,  obtained
...
* Validation and cross-validation (CV) are tools to estimate the test MSE.
...
the test error, estimated by CV.
