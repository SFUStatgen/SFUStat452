...
pdat <- predDir(sfit,Smarket)
table(pdat)
mean(pdat$pred != pdat$obs) # enough to give partial variable names
```

The above prediction error is optimistic because
...
pdat <- predDir(sfit,Smarket.test)
table(pdat)
mean(pdat$pred != pdat$obs) 
```
...
preds <- predict(sfit,newdata=Smarket.test)
pdat <- data.frame(predDirection = preds$class,
                   obsDirection = Smarket.test$Direction)
table(pdat)
mean(pdat$pred != pdat$obs)
```

The error rate is about 44%.
...
pdat <-data.frame(
  predDirection=knn(train=train.X,test=test.X,cl=train.Direction,k=3),
  obsDirection=Smarket.test$Direction)
table(pdat)
mean(pdat$pred != pdat$obs)
```
