...
which classifis as $1$ if the weighted sum is $>0$ and $-1$ otherwise.
...
* If a regression tree and the loss squared-error, 
\begin{eqnarray*}
...
from step $m-1$.
* Solve (\ref{eqn:boostCrit}) by fitting a tree 
...
take $f_m(x) = f_{m-1}(x) + T(x;\gamma_m)$.
...
```{r}
hboost <- gbm(I(AHD=="Yes") ~ ., data=Heart[train,],
              n.trees=5000,distributio="bernoulli",shrinkage=.2)
boo.hpred <- predict(hboost,newdata=Heart[-train,],
                     n.trees=5000,type="response")
boo.hpred <- (boo.hpred>0.5)
table(boo.hpred,Heart[-train,]$AHD)
