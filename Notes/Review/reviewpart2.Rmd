...
```{r}
library(leaps)
rr <- regsubsets(HUIDHSI ~ .,data=hsred,nvmax=40,
                 method="forward")
ss <- summary(rr)
which.min(ss$bic)
...
```{r}
Y <- hs$HUIDHSI
Xfull <- model.matrix(HUIDHSI ~ .,data=hsred)
Xred <- Xfull[,ss$which[30,]]
ll <- lm.fit(Xred,Y)
```
...
library(glmnet)
Y <- hs$HUIDHSI
Xfull <- model.matrix(HUIDHSI ~ .,data=hsred)[,-1]
Xfull <- scale(Xfull)
lambdas <- 10^{seq(from=-3,to=5,length=100)}
ll <- glmnet(Xfull,Y,alpha=1,lambda=lambdas)
