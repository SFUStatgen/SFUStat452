...
* Three sources:
    1. Model bias: The model $f(x_0)$ may be wrong. We will 
    ignore this for now; i.e., assume Bias($\hat{f}(x_0)$)=0.
    ...
    3. Irreducible error: $Y = f(x_0) + \epsilon$, and 
    so even if $\hat{f} = f$, predictions will not be perfect.
* We construct confidence intervals for $f(x_0)$  to 
...
* For fixed $x_0$, $f(x_0)$ is a function of parameters, and 
so is a parameter. 
* We can construct a confidence interval for $f(x_0)$ based
on the sampling distribution of $\hat{f}(x_0)$. 
