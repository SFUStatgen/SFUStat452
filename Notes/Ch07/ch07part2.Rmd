...
cubic polynomials, constrained so that the resulting function
is continuous at all knots, with continuous first and second 
derivative.
* There are several bases for such functions.
    * A basis is a set of "axes" that can be combined to 
    make any function.
* Text (page 273) uses the truncated power basis. 
    + They show that $K$ interior knots leads to $K+3$ basis functions,
    so $K+4$ coefficients, or degrees of freedom (df).
* An alternative basis is the B-spline basis.
...
* Note: The basis functions are not
restricted to the intervals defined by the knots
...
outside the knots, and these constraints have the effect
